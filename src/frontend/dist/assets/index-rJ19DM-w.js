...
const BOMB_FUSE_TIME = 1800;
const BOMB_RADIUS = 160;
const BOMB_REPLENISH_FRAMES = 1800;
const HEROES = [
  {
...
function circleRect(cx, _cy, cr, rx, ry, rw, rh) {
...
function getLevelConfig(level) {
  return {
    ...
    health: 40 + level * 20,
    speed: Math.min(1.2 + level * 0.15, 3),
    aggressionDist: Math.min(180 + level * 30, 600),
    shootInterval: Math.max(1200 - level * 100, 250),
    bossLevel: [3, 5, 7, 9].indexOf(level) >= 0 ? [3, 5, 7, 9].indexOf(level) : -1
  };
}
function playSound(ctx, muted, type) {
  if (!ctx || muted) return;
  try {
    ...
    if (type === "shoot") {
      ...
      osc.frequency.exponentialRampToValueAtTime(50, now2 + 0.1);
      gain.gain.setValueAtTime(0.2, now2);
      gain.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.12);
      osc.start(now2);
      osc.stop(now2 + 0.12);
    } else if (type === "death") {
      osc.type = "sawtooth";
      ...
      gain.gain.setValueAtTime(0.3, now2);
      gain.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.4);
      osc.start(now2);
      osc.stop(now2 + 0.4);
    } else if (type === "boss") {
      osc.type = "sawtooth";
      osc.frequency.setValueAtTime(110, now2);
      osc.frequency.setValueAtTime(220, now2 + 0.1);
      osc.frequency.setValueAtTime(110, now2 + 0.2);
      gain.gain.setValueAtTime(0.25, now2);
      gain.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.5);
      osc.start(now2);
      osc.stop(now2 + 0.5);
    } else if (type === "playerhit") {
      osc.type = "sine";
      osc.frequency.setValueAtTime(600, now2);
      osc.frequency.exponentialRampToValueAtTime(100, now2 + 0.2);
      gain.gain.setValueAtTime(0.2, now2);
      ...
      osc.frequency.exponentialRampToValueAtTime(200, now2 + 0.3);
      gain.gain.setValueAtTime(0.12, now2);
      gain.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.35);
      osc.start(now2);
      osc.stop(now2 + 0.35);
      ...
      try {
        ...
        g2.gain.setValueAtTime(0.6, now2);
        g2.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.5);
        src.connect(g2);
      ...
      gain.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.06);
      osc.start(now2);
      osc.stop(now2 + 0.06);
    }
  } catch (_) {
...
function startBgMusic(ctx, muted) {
...
function drawCharacter(ctx, x, y, _bodyColor, _legColor, _accentColor, facing, _isMoving, _animFrame, scale = 1, heroIdx = 0) {
...
function drawEnemy(ctx, enemy, cx, _cy, screenX, screenY) {
  const dx = cx - enemy.x;
...
function drawParkedCar(ctx, x, y, color, scale = 1) {
  const w = 70 * scale;
  const h = 30 * scale;
  ctx.fillStyle = color;
  ctx.beginPath();
...
function drawMap(ctx, state, camX, camY, vw, vh) {
  const now2 = Date.now();
  const warLevel = state.currentLevel;
  const VPX = vw / 2;
  const VPY = vh * 0.33;
  const cityBgImg = _charImgs.cityBg;
  if ((cityBgImg == null ? void 0 : cityBgImg.complete) && cityBgImg.naturalWidth > 0) {
  ...
  if ((militaryBaseImg == null ? void 0 : militaryBaseImg.complete) && militaryBaseImg.naturalWidth > 0) {
  ...
  if (warLevel >= 5) {
    ...
    for (let sci = 0; sci < smokePositions.length; sci++) {
      ...
      for (let si = 0; si < 5; si++) {
        const rise = (now2 * 0.035 + si * 30 + sci * 100) % 100;
        const ssx = scx + Math.sin(now2 * 1e-3 + si + sci) * 10;
        const ssy = VPY - 20 - rise;
        const sr = 10 + si * 4;
        ctx.save();
        ctx.globalAlpha = 0.12 * (1 - rise / 100) * Math.min(1, (warLevel - 4) * 0.5);
        ctx.fillStyle = "#333";
        ctx.beginPath();
  ...
  for (const bb of state.burningBuildings) {
    ...
    for (let fi = 0; fi < 5; fi++) {
      const fx = fireCenterX + (fi - 2) * 22 + Math.sin(now2 * 7e-3 + fi) * 8;
      const fy = fireCenterY + Math.sin(now2 * 5e-3 + fi * 1.4) * 8;
      const fh = 38 + Math.sin(now2 * 9e-3 + fi) * 12;
      ...
      ctx.globalAlpha = fireAlpha * (0.6 + Math.sin(now2 * 0.01 + fi) * 0.3);
      ctx.fillStyle = fg2;
      ctx.beginPath();
  ...
  for (const e of state.enemies) {
    ...
    ctx.save();
    ctx.globalAlpha = 0.08 + 0.04 * Math.sin(now2 * 3e-3);
...
function drawHUD(ctx, state, heroName, _heroBodyColor, vw, vh) {
  ctx.save();
...
function drawJoystick(ctx, joy, _vw, vh) {
  if (!joy.active) {
    ctx.save();
...
function drawFireButton(ctx, vw, vh, bombCount) {
  ctx.save();
  ctx.globalAlpha = 0.7;
  const bx = vw - 80;
  const by = vh - 110;
...
function worldToAlleyScreen(worldX, worldY, playerX, playerY, vw, vh) {
  const VPX = vw / 2;
  const VPY = vh * 0.33;
  ...
  const perspDiv = Math.max(1, depth * 7e-3);
  ...
  const visible = sx > -160 && sx < vw + 160 && sy > VPY - 20 && sy < GROUND_Y + 50;
  return { sx, sy, scale, visible };
}
function GameCanvas() {
  const canvasRef = reactExports.useRef(null);
  const [gamePhase, setGamePhase] = reactExports.useState("menu");
  const [selectedHero, setSelectedHero] = reactExports.useState(0);
  ...
  const phaseRef = reactExports.useRef("menu");
  const bgMusicStopRef = reactExports.useRef(null);
  const selectedHeroRef = reactExports.useRef(0);
  const mobileFireRef = reactExports.useRef(false);
  const mobileBombRef = reactExports.useRef(false);
  reactExports.useEffect(() => {
    const imageMap = {
      ...
      sp_aslam: "/assets/generated/sp-aslam-realistic-transparent.dim_200x340.png",
      major_iqbal: "/assets/generated/major-iqbal-realistic-transparent.dim_200x340.png",
  ...
  const updateGame = reactExports.useCallback(
    (state, dt, canvas) => {
      const vw = canvas.width;
      const vh = canvas.height;
      const p = state.player;
      HEROES[selectedHeroRef.current];
      if (state.muted !== muted) state.muted = muted;
      let mvx = 0;
      let mvy = 0;
      if (state.keysDown.has("ArrowLeft") || state.keysDown.has("a") || state.keysDown.has("A"))
        mvx -= 1;
      if (state.keysDown.has("ArrowRight") || state.keysDown.has("d") || state.keysDown.has("D"))
        mvx += 1;
      if (state.keysDown.has("ArrowUp") || state.keysDown.has("w") || state.keysDown.has("W"))
        mvy -= 1;
      if (state.keysDown.has("ArrowDown") || state.keysDown.has("s") || state.keysDown.has("S"))
        mvy += 1;
      ...
      p.shootTimer -= dt;
      const wantsShoot = state.keysDown.has(" ") || state.keysDown.has("Enter") || mobileFireRef.current;
      ...
      if (state.frameCount % BOMB_REPLENISH_FRAMES === 0 && state.frameCount > 0) {
        state.bombCount = Math.min(5, state.bombCount + 1);
      }
      const wantsBomb = state.keysDown.has("b") || state.keysDown.has("B") || mobileBombRef.current;
      ...
      for (const bomb of state.bombs) {
        ...
        if (!bomb.landed) {
          ...
          if (hitBuilding || bomb.x < 0 || bomb.x > MAP_W || bomb.y < 0 || bomb.y > MAP_H) {
            bomb.landed = true;
          }
          ...
          bomb.fuseTimer -= dt;
          if (bomb.fuseTimer < 600 && bomb.fuseTimer > 0 && state.frameCount % 12 === 0) {
          ...
          if (bomb.fuseTimer <= 0) {
            ...
            for (let i = 0; i < 50; i++) {
              ...
              state.particles.push({
                ...
                maxLife: 1e3,
                color: ["#FF6D00", "#FFD600", "#FF5252", "#FFFF00", "#FF8C00"][Math.floor(Math.random() * 5)],
      ...
      for (const e of state.enemies) {
        ...
        if (e.state === "idle") {
          ...
          if (pd < 25 || e.stateTimer > 1800) {
            const r = seededRand(
              e.id + Math.floor(e.stateTimer * 2e-3 + state.frameCount * 0.01)
            );
        ...
        const speed = e.state === "flee" ? ENEMY_FLEE_SPEED : e.state === "fight" ? ENEMY_FIGHT_SPEED : ENEMY_PATROL_SPEED;
      ...
      for (const b of state.bullets) {
        ...
        if (b.dist > BULLET_RANGE || b.x < 0 || b.x > MAP_W || b.y < 0 || b.y > MAP_H) {
          b.alive = false;
          continue;
        }
                ...
                if (e.isBoss) {
                  ...
                  for (let i = 0; i < 20; i++) {
                    state.particles.push({
                      ...
                      life: 800,
                      maxLife: 800,
                      color: ["#FFD700", "#FF6D00", "#FF5252", "#76FF03"][Math.floor(Math.random() * 4)],
                      size: 6
                    });
                  }
                } else {
  ...
  const renderGame = reactExports.useCallback(
    (state, canvas) => {
      ...
      for (const b of state.bullets) {
        ...
        ctx.strokeStyle = b.fromPlayer ? "rgba(255,214,0,0.4)" : "rgba(255,23,68,0.4)";
        ctx.lineWidth = 2;
        ctx.stroke();
      }
      const now2 = Date.now();
      for (const bomb of state.bombs) {
        ...
        ctx.stroke();
        if (bomb.landed) {
          const sparkActive = Math.sin(now2 * 0.025) > 0;
          if (sparkActive) {
          ...
          ctx.strokeStyle = fuseRatio > 0.5 ? "#4CAF50" : fuseRatio > 0.25 ? "#FF9800" : "#FF1744";
          ctx.lineWidth = 3;
          ctx.beginPath();
          ctx.arc(
            bsx,
      ...
      if (state.levelTransition) {
        ...
        const subtitle = levelSubtitles[state.currentLevel] || `Wave ${state.currentLevel} — Eliminate All Threats`;
  ...
  reactExports.useEffect(() => {
    ...
    const onTouchStart = (e) => {
      ...
      for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (Math.abs(t.clientX - (vw - 28)) < 24 && Math.abs(t.clientY - 28) < 24) {
          setMuted((m) => {
            const nm = !m;
        ...
        if (Math.abs(t.clientX - (vw - 80)) < 55 && Math.abs(t.clientY - (vh - 110)) < 55) {
          mobileFireRef.current = true;
          continue;
        }
        if (Math.abs(t.clientX - (vw - 155)) < 45 && Math.abs(t.clientY - (vh - 90)) < 45) {
  ...
  reactExports.useEffect(() => {
    ...
    const onClick = (e) => {
      const vw = canvas.width;
      if (Math.abs(e.clientX - (vw - 28)) < 24 && Math.abs(e.clientY - 28) < 24) {
        setMuted((m) => {
          const nm = !m;
    ...
    if (gamePhase === "win") {
      for (let i = 0; i < 60; i++) {
        fireworkParticles.push({
          ...
          life: 1500 + Math.random() * 1e3,
          maxLife: 2500,
          color: ["#FFD700", "#FF6D00", "#76FF03", "#FF5252", "#40C4FF"][Math.floor(Math.random() * 5)],
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        ...
        gamePhase === "menu" && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            className: "absolute inset-0 flex flex-col items-center justify-center",
            style: {
              background: "linear-gradient(180deg, #0D1B2A 0%, #1B2838 40%, #2D1B00 100%)"
            },
            children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center px-6 max-w-lg", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "p",
                {
                  ...
                  children: "Hunt down the 4 bosses and save your country"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "p",
                {
                  ...
                  children: "💣 NEW: Throw bombs to destroy buildings with enemies inside!"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: "WASD / Joystick to move · SPACE / FIRE to shoot · B / 💣 to bomb"
                }
              )
            ] })
          }
        ),
        gamePhase === "select" && /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            className: "absolute inset-0 flex flex-col items-center justify-center",
            style: {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "h2",
                {
                  ...
                  style: { color: "#FFD600" },
                  children: "CHOOSE YOUR HERO"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-8 flex-wrap justify-center px-4", children: HEROES.map((hero, idx) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  type: "button",
                  onClick: () => {
                  ...
                  style: {
                    background: selectedHero === idx ? "rgba(255,214,0,0.15)" : "rgba(255,255,255,0.06)",
                    border: selectedHero === idx ? "2px solid #FFD600" : "2px solid rgba(255,255,255,0.12)",
                    minWidth: 160
                  },
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        ...
                        children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-4xl", children: idx === 0 ? "🧔" : "🕵️" })
                      }
                    ),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
        ...
        (gamePhase === "win" || gamePhase === "gameover") && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            className: "absolute inset-0 flex flex-col items-center justify-center",
