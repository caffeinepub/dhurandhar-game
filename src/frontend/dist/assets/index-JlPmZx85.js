...
function stopAllMusic() {
  ...
  for (const n of musicNodes) {
    try {
      (_a2 = n.stop) == null ? void 0 : _a2.call(n);
    } catch {
    }
  }
  musicNodes = [];
...
function playNote(ctx, dest, freq, startTime, dur, vol, type = "square", attack = 0.01, decay = 0.05, sustain = 0.7) {
...
function playDrum(ctx, dest, startTime, isKick) {
  const buf = ctx.createBuffer(1, ctx.sampleRate * 0.18, ctx.sampleRate);
  const d = buf.getChannelData(0);
  for (let i = 0; i < d.length; i++) {
    const t = i / ctx.sampleRate;
    d[i] = isKick ? Math.sin(2 * Math.PI * 80 * Math.exp(-t * 30) * t) * Math.exp(-t * 20) : (Math.random() * 2 - 1) * Math.exp(-t * 40) * 0.7;
  }
...
function scheduleBar(ctx, dest, startTime, level) {
  const bpm = 90 + Math.min(level * 4, 50);
  const beat = 60 / bpm;
  const bar = beat * 4;
  const intensity = Math.min((level - 1) / 19, 1);
  ...
  if (level >= 3) {
    const mel = intensity > 0.5 ? MELODY_HIGH : intensity > 0.25 ? MELODY_MID : MELODY_LOW;
    for (let i = 0; i < 8; i++) {
      const t = startTime + i * (bar / 8) + beat * 0.02;
      const skip = intensity < 0.3 && i % 2 === 1;
      if (!skip) {
        playNote(
          ctx,
          dest,
  ...
  if (level >= 6) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      for (const f of chordFreqs) {
        playNote(ctx, dest, f, t, beat * 0.3, 0.06, "square", 5e-3, 0.02, 0.4);
      }
    }
  }
  if (level >= 2) {
...
function startMusicLoop(level) {
  ...
  try {
    let loop = function() {
      ...
      musicScheduler = setTimeout(loop, Math.max(0, (lookahead - 0.1) * 1e3));
    };
...
function generateCity(theme, seed) {
...
function generateNPCs(seed) {
  ...
  const types = ["man", "soldier", "woman", "man"];
...
function spawnLevelEnemies(level, idStart) {
...
function spawnPickups(level, buildings, idStart) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      const overlap = buildings.some(
        (b) => px > b.x - 20 && px < b.x + b.w + 20 && py > b.y - 20 && py < b.y + b.h + 20
      );
      if (!overlap) break;
...
function spawnFloatingText(s, x, y, text, color) {
...
function throwBomb(s) {
  ...
  if (alive.length > 0) {
    const nearest = alive.reduce(
      (best, e) => Math.hypot(e.x - s.hx, e.y - s.hy) < Math.hypot(best.x - s.hx, best.y - s.hy) ? e : best
    );
    tx = nearest.x;
    ty = nearest.y;
  }
...
function spawnParticles(s, sx, sy, color, count) {
  for (let i = 0; i < count; i++) {
...
function drawNPCTopDown(ctx, cx, cy, type, hint, showBubble) {
  ctx.save();
  const r = 14;
  ctx.beginPath();
...
function drawCity(ctx, camX, camY, vw, vh, theme, buildings, level) {
  const roadColors = {
    morning: "#888070",
    dusk: "#5a4a3a",
    night: "#1a1818",
  ...
  for (const b of buildings) {
    ...
    const textColor = b.type === "hospital" ? "#ff4444" : b.type === "police" ? "#4488ff" : b.type === "mosque" ? "#44ff88" : "#ffffaa";
    ...
    ctx.fillStyle = theme === "night" || theme === "final" ? "rgba(255,200,100,0.7)" : "rgba(180,200,255,0.5)";
  ...
  if (theme === "night" || theme === "final" || theme === "warzone") {
    ...
    const intensity = theme === "final" ? 0.35 : theme === "warzone" ? 0.2 : 0.15;
    if (level >= 9) {
      for (let i = 0; i < 4; i++) {
        const sx = (700 + i * 900 - camX) % vw;
        const alpha = (0.15 + Math.sin(t * 0.4 + i) * 0.06) * Math.min(1, (level - 8) / 4);
...
function GameCanvas() {
  ...
  const [phase, setPhase] = reactExports.useState("menu");
  const [hero, setHero] = reactExports.useState("humza");
  const [muted, setMuted] = reactExports.useState(false);
  const rafRef = reactExports.useRef(0);
  const lastTimeRef = reactExports.useRef(0);
  const joystickRef = reactExports.useRef({ active: false, cx: 0, cy: 0, dx: 0, dy: 0 });
  const spriteHumza = reactExports.useRef(null);
  const spriteAjay = reactExports.useRef(null);
  const spriteRehman = reactExports.useRef(null);
  const spriteSpAslam = reactExports.useRef(null);
  const spriteMajorIqbal = reactExports.useRef(null);
  const spriteJameel = reactExports.useRef(null);
  const spriteGrunt = reactExports.useRef(null);
  reactExports.useEffect(() => {
    const load = (src) => {
      const img = new Image();
      img.src = src;
  ...
  const startGame = reactExports.useCallback(
    (selectedHero) => {
      ...
      setPhase("playing");
      currentMusicLevel = ((_a2 = stateRef.current) == null ? void 0 : _a2.level) ?? 1;
      startDrone(muted);
    },
    [muted]
  );
  reactExports.useEffect(() => {
    ...
    function tick(timestamp) {
      ...
      const dt = Math.min((timestamp - lastTimeRef.current) / 1e3, 0.05);
  ...
  function update(s, dt) {
    ...
    if ((keys.has("Space") || keys.has("Enter")) && alive.length > 0) {
      const nearest = alive.reduce(
        (b, e) => Math.hypot(e.x - s.hx, e.y - s.hy) < Math.hypot(b.x - s.hx, b.y - s.hy) ? e : b
      );
    ...
    for (const b of s.bullets) {
      ...
      if (b.fromPlayer) {
        for (const e of s.enemies) {
          if (e.dead) continue;
          if (Math.hypot(b.x - e.x, b.y - e.y) < (e.isBoss ? BOSS_RADIUS : CHAR_RADIUS) + 6) {
            e.hp -= 20;
            b.life = 0;
        ...
        if (s.invincTimer <= 0 && Math.hypot(b.x - s.hx, b.y - s.hy) < HERO_RADIUS + 6) {
          s.hhp -= 15;
          b.life = 0;
          s.invincTimer = 0.5;
          s.screenShake = 0.3;
    ...
    for (const bomb of s.bombs) {
      ...
      if (bomb.timer <= 0) {
        ...
        for (const b of s.buildings) {
          if (Math.hypot(bomb.x - (b.x + b.w / 2), bomb.y - (b.y + b.h / 2)) < BOMB_RADIUS * 1.2) {
            b.onFire = true;
            b.fireTimer = 5;
          }
        }
  ...
  function render(ctx, vw, vh, s) {
    ...
    for (const e of s.enemies) {
      ...
      const enemyImg = e.isBoss ? bossSprite[e.name] ?? null : spriteGrunt.current;
      ...
      if ((enemyImg == null ? void 0 : enemyImg.complete) && enemyImg.naturalWidth > 0) {
    ...
    const heroAlpha = s.invincTimer > 0 ? Math.sin(s.invincTimer * 20) > 0 ? 0.4 : 1 : 1;
    const heroImg = s.hero === "humza" ? spriteHumza.current : spriteAjay.current;
    const hsw = 80;
    const hsh = 128;
    ctx.save();
    ...
    ctx.fillStyle = "rgba(0,0,0,0.4)";
    ctx.fill();
    const heroFacingLeft = s.hAngle > Math.PI / 2 && s.hAngle < 3 * Math.PI / 2;
    if (heroFacingLeft) {
      ctx.translate(heroSx, heroSy);
      ctx.scale(-1, 1);
      ctx.translate(-heroSx, -heroSy);
    }
    if ((heroImg == null ? void 0 : heroImg.complete) && heroImg.naturalWidth > 0) {
    ...
    if (s.muzzleFlashTimer > 0) {
      ...
      const muzzleX = heroSx + Math.cos(s.hAngle) * (heroFacingLeft ? -muzzleOffset : muzzleOffset);
    ...
    for (const p of s.pickups) {
      ...
      ctx.fillStyle = p.type === "health" ? "#fff" : p.type === "speed" ? "#001a1a" : "#1a0800";
      ctx.fill();
      ctx.strokeStyle = p.type === "health" ? "#ff4444" : p.type === "speed" ? "#00ffff" : "#ff9900";
      ctx.lineWidth = 2;
      ctx.stroke();
      ctx.fillStyle = p.type === "health" ? "#ff2222" : p.type === "speed" ? "#00ffff" : "#ff9900";
    ...
    if (s.streakBannerTimer > 0) {
      ...
      ctx.globalAlpha = sba;
      const bannerColor = s.scoreMultiplier >= 4 ? "#ff44ff" : s.scoreMultiplier >= 3 ? "#ff6600" : "#ffee00";
  ...
  function drawHUD(ctx, vw, vh, s) {
    ...
    if (s.killStreak >= 2) {
      ctx.fillStyle = s.killStreak >= 7 ? "#ff44ff" : s.killStreak >= 5 ? "#ff6600" : "#ffcc00";
  ...
  function drawMinimap(ctx, vw, vh, s) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative w-full h-screen overflow-hidden bg-black", children: [
    ...
    phase === "menu" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute inset-0 flex flex-col items-center justify-end pb-14 pointer-events-none", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pointer-events-auto flex flex-col items-center gap-5", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            className: `px-5 py-3 rounded-lg font-bold text-base transition-all border-2 ${hero === "humza" ? "bg-yellow-500 text-black border-yellow-300 scale-105" : "bg-black/60 text-yellow-400 border-yellow-600"}`,
            children: "Humza Ali Mazari"
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            className: `px-5 py-3 rounded-lg font-bold text-base transition-all border-2 ${hero === "ajay" ? "bg-yellow-500 text-black border-yellow-300 scale-105" : "bg-black/60 text-yellow-400 border-yellow-600"}`,
            children: "Ajay Sanyal"
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-yellow-600 text-sm text-center", children: [
        "WASD / Arrows to move in all directions • Space to fire • B for bomb",
        /* @__PURE__ */ jsxRuntimeExports.jsx("br", {}),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-yellow-500", children: "20 Levels • 4 Bosses • Major Iqbal is the Final Boss!" })
      ] })
    ] }) }),
    (phase === "win" || phase === "gameover") && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute inset-0 flex flex-col items-center justify-end pb-20 pointer-events-none", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        type: "button",
        onClick: () => {
        ...
        children: phase === "win" ? "PLAY AGAIN" : "TRY AGAIN"
      }
    ) }),
    phase === "playing" && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "div",
        {
          ...
          style: { touchAction: "none" },
          children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-12 h-12 rounded-full bg-yellow-500/30 border border-yellow-400/50" })
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "absolute bottom-16 right-4 flex flex-col gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
