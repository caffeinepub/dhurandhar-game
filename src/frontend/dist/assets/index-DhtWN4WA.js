...
function spawnEnemies(level, bosses, idStart) {
...
function GameCanvas() {
  ...
  const joystickRef = reactExports.useRef({
    active: false,
    cx: 0,
    cy: 0,
    dx: 0,
  ...
  function update(s, dt) {
    ...
    const vw = ((_a2 = canvasRef.current) == null ? void 0 : _a2.width) ?? window.innerWidth;
    const vh = ((_b2 = canvasRef.current) == null ? void 0 : _b2.height) ?? window.innerHeight;
    ...
    for (const b of s.buildings) {
      if (newHx - hw < b.x + b.w && newHx + hw > b.x && s.hy - hh < b.y + b.h && s.hy + hh > b.y)
        colX = true;
      if (s.hx - hw < b.x + b.w && s.hx + hw > b.x && newHy - hh < b.y + b.h && newHy + hh > b.y)
        colY = true;
    }
    ...
    for (const b of s.bullets) {
      ...
      if (b.fromPlayer) {
        ...
        if (s.invincTimer <= 0 && Math.abs(b.x - s.hx) < 30 && Math.abs(b.y - s.hy) < 50) {
          s.hhp -= 15;
          b.life = 0;
          s.invincTimer = 0.5;
          s.screenShake = 0.3;
    ...
    if (aliveGrunts.length === 0) {
      ...
      s.levelTitle = titles[Math.min(s.level - 1, titles.length - 1)] ?? "TOTAL WARFARE";
      s.levelSplash = 3;
  ...
  function render(ctx, vw, vh, s, imgs) {
    const dpr = window.devicePixelRatio || 1;
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    ctx.imageSmoothingEnabled = true;
    ctx.imageSmoothingQuality = "high";
    ctx.clearRect(0, 0, vw, vh);
    const shakeX = s.screenShake > 0 ? (Math.random() - 0.5) * s.screenShake * 12 : 0;
    const shakeY = s.screenShake > 0 ? (Math.random() - 0.5) * s.screenShake * 8 : 0;
    ctx.save();
  ...
  function drawCharacter(ctx, imgs, key, sx, sy, w, h, facingLeft, fallbackColor) {
    const img = imgs[key];
    ctx.save();
    if (facingLeft) {
      ctx.scale(-1, 1);
  ...
  function drawHUD(ctx, vw, vh, s, _imgs) {
    ...
    ctx.fillStyle = hpRatio > 0.6 ? "#00e060" : hpRatio > 0.3 ? "#ffaa00" : "#ff2200";
  ...
  function drawNPC(ctx, sx, sy, type) {
    ctx.save();
    if (type === "soldier") {
  ...
  function drawSpeechBubble(ctx, sx, sy, text) {
    const maxW = 200;
    ctx.font = "11px Arial";
    const lines = [];
  ...
  function drawV16City(ctx, vw, vh, camX, levelRatio) {
    const groundY = vh * GROUND_Y_RATIO;
    const farColors = levelRatio > 0.5 ? ["#5a2010", "#4a1808", "#6a2818"] : ["#c8a060", "#b89050", "#d4b870"];
    for (let i = 0; i < 14; i++) {
      const bx = ((i * 170 - camX * 0.08) % (vw + 200) + vw + 200) % (vw + 200) - 100;
      const bh = 100 + i % 4 * 50;
      ctx.fillStyle = farColors[i % farColors.length];
      ctx.fillRect(bx, groundY - bh, 150, bh);
      ctx.fillStyle = "rgba(0,0,0,0.2)";
      ctx.fillRect(bx, groundY - bh, 150, 8);
      ctx.fillStyle = levelRatio > 0.5 ? "rgba(255,100,0,0.4)" : "rgba(255,240,180,0.5)";
    ...
    const midColors = levelRatio > 0.5 ? ["#7a3020", "#6a2010", "#8a3828"] : ["#e8b870", "#d09858", "#f0c880"];
    for (let i = 0; i < 10; i++) {
      const bx = ((i * 220 + 80 - camX * 0.15) % (vw + 250) + vw + 250) % (vw + 250) - 125;
      const bh = 130 + i % 3 * 70;
      ...
      ctx.fillStyle = levelRatio > 0.5 ? "rgba(255,80,0,0.5)" : "rgba(255,230,120,0.6)";
    ...
    const nearColors = levelRatio > 0.5 ? ["#3a1008", "#4a1810"] : ["#a07840", "#b88848"];
    for (let i = 0; i < 6; i++) {
      const bx = ((i * 300 + 150 - camX * 0.25) % (vw + 300) + vw + 300) % (vw + 300) - 150;
      const bh = 160 + i % 2 * 80;
  ...
  function drawFire(ctx, x, y, width) {
  ...
  function handleCanvasTap(e) {
    var _a2, _b2;
    const vw = ((_a2 = canvasRef.current) == null ? void 0 : _a2.width) ?? window.innerWidth;
    ((_b2 = canvasRef.current) == null ? void 0 : _b2.height) ?? window.innerHeight;
    const x = e.clientX;
    const y = e.clientY;
    if (x > vw - 50 && y < 70) {
      handleMuteBtn();
  ...
  reactExports.useEffect(() => {
    ...
    function drawMenu() {
      ...
      if ((cityImg == null ? void 0 : cityImg.complete) && cityImg.naturalWidth > 0) {
      ...
      if ((groundImg == null ? void 0 : groundImg.complete) && groundImg.naturalWidth > 0) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative w-full h-screen overflow-hidden bg-black", children: [
    ...
    phase === "menu" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute inset-0 flex flex-col items-center justify-end pb-16 pointer-events-none", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pointer-events-auto flex flex-col items-center gap-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            className: `px-6 py-3 rounded-lg font-bold text-lg transition-all border-2 ${hero === "humza" ? "bg-yellow-500 text-black border-yellow-300 scale-105" : "bg-black/60 text-yellow-400 border-yellow-600 hover:bg-black/80"}`,
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            className: `px-6 py-3 rounded-lg font-bold text-lg transition-all border-2 ${hero === "ajay" ? "bg-yellow-500 text-black border-yellow-300 scale-105" : "bg-black/60 text-yellow-400 border-yellow-600 hover:bg-black/80"}`,
            style: { fontFamily: "'Cinzel', serif" },
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-yellow-600 text-sm", children: "WASD / Arrow Keys to move • Space to fire • B for bomb" })
    ] }) }),
    ...
    phase === "playing" && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "absolute bottom-8 right-4 flex flex-col gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
