...
function drawHeroTopDown(ctx, cx, cy, angle, isHumza, alpha, radius) {
  ctx.save();
  ...
  for (const s of [-1, 1]) {
    const ex = cx + faceDx + Math.cos(perp) * s * radius * 0.15 + Math.cos(angle) * radius * 0.1;
    const ey = cy + faceDy + Math.sin(perp) * s * radius * 0.15 + Math.sin(angle) * radius * 0.1;
    ctx.beginPath();
...
function drawEnemyTopDown(ctx, cx, cy, angle, color, accentColor, isBoss, radius, hp, maxHp, name, alertTimer) {
  ctx.save();
  ctx.beginPath();
  ...
  for (const side of [-1, 1]) {
    const ex = cx + faceDx + Math.cos(perp) * side * radius * 0.14 + Math.cos(angle) * radius * 0.08;
    const ey = cy + faceDy + Math.sin(perp) * side * radius * 0.14 + Math.sin(angle) * radius * 0.08;
    ctx.beginPath();
    ctx.arc(ex, ey, eyeR, 0, Math.PI * 2);
