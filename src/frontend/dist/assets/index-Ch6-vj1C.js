...
function generateBuildings(theme) {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    buildings.push({
      ...
      onFire: false,
      fireTimer: 0,
      sign: rand() > 0.5 ? signs[Math.floor(rand() * signs.length)] : void 0
    });
  }
  return buildings;
}
function generateNPCs() {
  const positions = [500, 1200, 2e3, 2800];
...
function throwBomb(s) {
  ...
  if (alive.length > 0) {
    const nearest = alive.reduce(
      (best, e) => Math.abs(e.x - s.hx) < Math.abs(best.x - s.hx) ? e : best
    );
    tx = nearest.x;
  }
  s.bombs.push({
...
function drawBackground(ctx, vw, vh, groundY, camX, theme, buildings) {
  ...
  for (let i = 0; i < 14; i++) {
    const bx = ((i * 170 - camX * 0.06) % (vw + 200) + vw + 200) % (vw + 200) - 100;
    const bh = 90 + i % 4 * 45;
    ctx.fillStyle = farColors[theme][i % 2];
    ctx.fillRect(bx, groundY - bh, 150, bh);
    const winColor = theme === "night" ? "rgba(255,180,0,0.6)" : theme === "final" ? "rgba(255,60,0,0.5)" : "rgba(255,220,130,0.4)";
  ...
  for (let i = 0; i < 10; i++) {
    const bx = ((i * 220 + 80 - camX * 0.14) % (vw + 260) + vw + 260) % (vw + 260) - 130;
    const bh = 120 + i % 3 * 65;
    ctx.fillStyle = midColors[theme][i % 2];
    ctx.fillRect(bx, groundY - bh, 180, bh);
    ctx.fillStyle = "rgba(0,0,0,0.15)";
    ctx.fillRect(bx + 162, groundY - bh, 18, bh);
    const winClr = theme === "night" ? "rgba(255,140,0,0.65)" : theme === "final" ? "rgba(255,40,0,0.6)" : "rgba(255,220,100,0.5)";
  ...
  for (let i = 0; i < 5; i++) {
    const bx = ((i * 320 + 160 - camX * 0.22) % (vw + 320) + vw + 320) % (vw + 320) - 160;
    const bh = 150 + i % 2 * 90;
  ...
  ctx.strokeStyle = theme === "night" || theme === "final" ? "rgba(255,255,255,0.12)" : "rgba(255,255,255,0.18)";
  ctx.lineWidth = 3;
  ...
  haze.addColorStop(
    1,
    theme === "night" ? "rgba(40,10,0,0.4)" : theme === "final" ? "rgba(60,0,0,0.5)" : "rgba(100,60,20,0.3)"
  );
  ctx.fillStyle = haze;
  ctx.fillRect(0, groundY - 50, vw, 50);
}
function drawCharFallback(ctx, cx, gy, facingLeft, isHero, isBoss, alpha = 1) {
  ctx.save();
...
function drawNPC(ctx, cx, gy, type) {
  ctx.save();
...
function GameCanvas() {
  ...
  const [imgsLoaded, setImgsLoaded] = reactExports.useState(false);
  ...
  const imgsRef = reactExports.useRef({
    humza: null,
    ajay: null,
    rehman: null,
    sp_aslam: null,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative w-full h-screen overflow-hidden bg-black", children: [
    ...
    !imgsLoaded && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "absolute inset-0 flex flex-col items-center justify-center bg-black/80 z-50", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-yellow-400 font-bold text-2xl animate-pulse", children: "Loading Characters..." }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-yellow-600 text-sm mt-2", children: "Preparing Dhurandhar" })
    ] }),
    phase === "menu" && imgsLoaded && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute inset-0 flex flex-col items-center justify-end pb-14 pointer-events-none", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pointer-events-auto flex flex-col items-center gap-5", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-yellow-600 text-sm text-center", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-yellow-500", children: "20 Levels \\u2022 4 Bosses \\u2022 Save the Country!" })
      ] })
    ] }) }),
