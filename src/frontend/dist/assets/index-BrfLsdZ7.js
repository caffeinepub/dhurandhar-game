...
function drawHumza(ctx, cx, gy, facingLeft, alpha = 1) {
  ctx.save();
...
function drawAjay(ctx, cx, gy, facingLeft, alpha = 1) {
  ctx.save();
  ctx.globalAlpha = alpha;
  if (facingLeft) {
    ctx.scale(-1, 1);
...
function drawGrunt(ctx, cx, gy, facingLeft) {
  ctx.save();
  if (facingLeft) {
    ctx.scale(-1, 1);
    cx = -cx;
...
function drawBoss(ctx, cx, gy, facingLeft, name) {
  ctx.save();
  if (facingLeft) {
    ctx.scale(-1, 1);
    cx = -cx;
  ...
  if (name.includes("Rehman")) {
    ...
    ctx.fillRect(cx - 22 * s, gy - 92 * s, 44 * s, 6 * s);
    ctx.fillStyle = "#5a1010";
    ctx.fillRect(cx - 38 * s, gy - 90 * s, 18 * s, 32 * s);
    ctx.fillRect(cx + 20 * s, gy - 90 * s, 18 * s, 32 * s);
...
function GameCanvas() {
  ...
  function render(ctx, vw, vh, s) {
    ...
    const shakeX = s.screenShake > 0 ? (Math.random() - 0.5) * s.screenShake * 10 : 0;
    ...
    for (const b of s.buildings) {
      ...
      ctx.fillStyle = levelRatio > 0.5 ? "rgba(255,80,0,0.45)" : "rgba(255,230,120,0.55)";
    ...
    if (s.level >= 5) {
      for (let i = 0; i < 3; i++) {
        const sx = ((i * 900 + 200 - s.cameraX * 0.2) % (vw + 300) + vw + 300) % (vw + 300) - 150;
  ...
  function drawV16City(ctx, vw, _vh, groundY, camX, lr) {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      ctx.fillStyle = lr > 0.5 ? "rgba(255,80,0,0.35)" : "rgba(255,230,130,0.45)";
    ...
    for (let i = 0; i < 10; i++) {
      ...
      ctx.fillStyle = lr > 0.5 ? "rgba(255,70,0,0.5)" : "rgba(255,220,100,0.55)";
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative w-full h-screen overflow-hidden bg-black", children: [
    ...
    phase === "menu" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute inset-0 flex flex-col items-center justify-end pb-14 pointer-events-none", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pointer-events-auto flex flex-col items-center gap-5", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-yellow-600 text-sm", children: "A/D or Arrow Keys to move • Space to fire • B for bomb" })
    ] }) }),
