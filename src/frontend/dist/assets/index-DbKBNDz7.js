...
function drawCharacter(ctx, x, y, _bodyColor, _legColor, _accentColor, facing, isMoving, animFrame, scale = 1, heroIdx = 0) {
  ctx.save();
  ctx.translate(x, y);
  ...
  if (heroIdx === 0) {
    ...
    ctx.fill();
    ctx.fillStyle = "#333";
    ctx.fillRect(15 * s, 3 * s, 18 * s, 1.5 * s);
    ctx.fillStyle = "#222";
    ctx.fillRect(30 * s, 1.5 * s, 2 * s, 3 * s);
    ctx.fillStyle = "#111";
    ctx.fillRect(34 * s, 4 * s, 9 * s, 2 * s);
    ctx.fillStyle = "#333";
    ctx.beginPath();
    ...
    ctx.fill();
    ctx.fillStyle = "#252525";
    ctx.fillRect(15 * s, 3 * s, 22 * s, 2 * s);
    ctx.fillStyle = "#111";
    ctx.beginPath();
...
function drawEnemy(ctx, enemy, cx, _cy, screenX, screenY) {
  ...
  if (!isBoss) {
    ...
    ctx.fillStyle = "#111";
    ctx.fillRect(34 * s, 3 * s, 9 * s, 2.5 * s);
    ctx.fillStyle = "#333";
    ctx.beginPath();
...
function drawMap(ctx, state, camX, camY, vw, vh) {
  ...
  if (warLevel <= 3) {
    ...
    for (const cl of cloudDefs) {
      ...
      for (let ci = -1; ci <= 1; ci++) {
        ctx.beginPath();
        ctx.arc(
          cl.x + ci * cl.r * 0.65 + Math.sin(now2 * 2e-4 + cl.t) * 3,
  ...
  for (const w of leftWins) {
    ...
    ctx.fillStyle = isLit ? warLevel > 4 ? "rgba(255,60,0,0.8)" : "rgba(255,215,80,0.85)" : "rgba(30,20,10,0.6)";
  ...
  for (const db of depthBldgs) {
    ...
    ctx.fillStyle = warLevel > 4 ? "rgba(255,80,0,0.6)" : "rgba(255,220,80,0.6)";
